...
internal fun previewReceivables(
    ...
    val pending = if (pendingAmount > 0.0) listOf(
        previewReceivable(orderId * 10 + 1, pendingAmount, OrderReceivableItemStatus.PENDING, "credito"),
    ) else emptyList()
    return paid + pending
}
...
internal fun previewReceivable(
    ...
    status = status,
    paymentMethod = PaymentMethod(id, paymentType, if (paymentType == "credito") 3 else 1, null, paymentType),
